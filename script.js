...
autoFillBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    appendLog(
      "❌ " + token.slice(0, 10) + "*****\x20-\x20エラー：" + error.message,
    );
  }
});
...
async function sendMessage(token, channelId, content, options = {}) {
  ...
  if (options.randomMentions) {
    const randomUser =
      options.randomMentions[
        Math.floor(Math.random() * options.randomMentions.length)
      ];
...
async function sendMessageWithRetry(
  ...
  while (retries < maxRetries) {
    try {
      ...
      if (response.ok) {
        appendLog(
          "✅ " + token.slice(0, 10) + "*****\x20-\x20メッセージ送信成功",
        );
        return true;
  ...
  appendLog(
    "❌ トークン(" + token.slice(0, 10) + ")\x20最大リトライ回数に達しました",
  );
  return false;
}
...
async function forwardMessageWithRetry(
  ...
  while (retries < maxRetries) {
    try {
      ...
      if (response.ok) {
        appendLog(
          "✅ " + token.slice(0, 10) + "*****\x20-\x20メッセージ転送成功",
        );
        return true;
      } else if (response.status === 429) {
        const data = await response.json();
...
form.addEventListener("submit", async (event) => {
  ...
  const mentionIds = mentionInput.value.trim()
    ? parseList(mentionInput.value)
    : null;
  const pollTitle = pollTitleInput.value.trim() || null;
  const pollAnswers = pollAnswersInput.value.trim()
    ? parseList(pollAnswersInput.value)
    : null;
    ...
    const forwardTasks = tokens.map((token) => {
      return async () => {
        ...
        while (!shouldStopSpam && totalSent < limit) {
          ...
          const success = await forwardMessageWithRetry(
            token,
            channelId,
            messageRef,
          );
